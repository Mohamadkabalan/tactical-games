...
class FrmExportViewCSVController {
	...
	private static function get_atts_for_export( $view_id, $download, $is_cron ) {
		...
		$view        = FrmProDisplay::getOne( $view_id, false, true );
	...
	public static function get_entry_ids_and_where( $view ) {
		...
		return FrmProDisplaysController::get_display_data( $view, '', false, $extra_atts );
	}
	...
	private static function create_file_for_download( $atts ) {
		...
		self::add_content_to_file( $atts['view'], $atts['entry_ids'], $file );
	 ...
	 * @param resource $file      File opened by fopen.
	 */
	private static function add_content_to_file( $view, $entry_ids, $file ) {
		$separator = FrmExportViewCSVHelper::get_column_separator();
		self::add_header_to_file( $view, $entry_ids, $file, $separator );
		self::add_inner_content_to_file( $view, $entry_ids, $file, $separator );
	}
	 ...
	 */
	private static function add_header_to_file( $view, $entry_ids, $file, $separator ) {
		...
		$before_content = FrmProDisplaysController::get_before_content_for_listing_page( $view, $args );
		$before_content = apply_filters( 'frm_export_csv_table_heading', $before_content, $view );
		$before_content = do_shortcode( $before_content );
		self::add_row_content_to_file( $before_content, $file, 'th', $separator );
	}
	 ...
	 * @param string   $separator Single character that separates cells in CSV.
	 */
	private static function add_row_content_to_file( $content, $file, $cell_tag = 'td', $separator = ',' ) {
	 ...
	 */
	private static function add_inner_content_to_file( $view, $entry_ids, $file, $separator = ',', $number_in_batch = 30 ) {
		$total_entries = count( $entry_ids );
		...
		for ( $offset = 0; $offset < $total_entries; $offset = $offset + $number_in_batch ) {
			...
			$inner_content = FrmProDisplaysController::get_inner_content_for_listing_page( $view, $args );
			$inner_content = do_shortcode( $inner_content );
			self::add_row_content_to_file( $inner_content, $file, 'td', $separator );
		}
	}
	...
	private static function save_file_online( $atts ) {
		...
		$file = fopen( untrailingslashit( $upload_dir ) . '/' . $atts['filename'], 'w' );
		self::add_content_to_file( $atts['view'], $atts['entry_ids'], $file );
		fclose( $file );
