...
file.name+"</a>"}}});this.on("addedfile",function(file){var thumbnail=maybeGetExtensionThumbnail(file.name);processesRunning++;frmFrontForm.showSubmitLoading(form);if(false!==thumbnail)jQuery(file.previewElement).find(".dz-image img").attr("src",thumbnail)});this.on("removedfile",function(file){var fileCount=this.files.length;if(this.options.uploadMultiple===false&&fileCount<1)this.enable();if(file.accepted!==false&&uploadFields[i].uploadMultiple!==true)jQuery('input[name="'+fieldName+'"]').val("");
if(file.accepted!==false&&typeof file.mediaID!=="undefined"){jQuery(file.previewElement).remove();fileCount=this.files.length;this.options.maxFiles=uploadFields[i].maxFiles-fileCount}});setTimeout(fixDropzoneIconsOnDropzoneInit,0);if(typeof uploadFields[i].mockFiles!=="undefined")for(var f=0;f<uploadFields[i].mockFiles.length;f++){var mockFile={name:uploadFields[i].mockFiles[f].name,size:uploadFields[i].mockFiles[f].size,url:uploadFields[i].mockFiles[f].file_url,mediaID:uploadFields[i].mockFiles[f].id};
this.emit("addedfile",mockFile);this.emit("thumbnail",mockFile,uploadFields[i].mockFiles[f].url);this.emit("complete",mockFile);this.files.push(mockFile)}}})}function fixDropzoneIconsOnDropzoneInit(){var images,length,index,image,thumbnail;images=document.querySelectorAll(".dz-image img");length=images.length;for(index=0;index<length;++index){image=images[index];thumbnail=maybeGetExtensionThumbnail(image.getAttribute("src"));if(false!==thumbnail)image.setAttribute("src",thumbnail)}}function maybeGetExtensionThumbnail(path){var ext=
path.split(".").pop();if("pdf"===ext)return getProPluginUrl()+"/images/pdf.svg";else if(-1!==ext.indexOf("doc"))return getProPluginUrl()+"/images/doc.svg";else if(-1!==ext.indexOf("xls"))return getProPluginUrl()+"/images/xls.svg";return false}function getProPluginUrl(){var freePluginUrlSplitBySlashes=frm_js.images_url.split("/");freePluginUrlSplitBySlashes.pop();freePluginUrlSplitBySlashes.pop();freePluginUrlSplitBySlashes.push("formidable-pro");return freePluginUrlSplitBySlashes.join("/")}function filePreviewHTML(field){return'<div class="dz-preview dz-file-preview frm_clearfix">\n'+
...
doSingleCalculation(allCalcs,fieldKey,vals,triggerField)}function fieldCanDoCalc(fieldType){var canDoCalc=false;if(fieldType==="text"||fieldType==="hidden"||fieldType==="number")canDoCalc=true;return canDoCalc}function getFieldKey(fieldHtmlId,fieldName){var fieldKey=fieldHtmlId.replace("field_",""),newFieldKey="";if(isRepeatingFieldByName(fieldName)){var fieldKeyParts=fieldKey.split("-");for(var i=0;i<fieldKeyParts.length-1;i++)if(newFieldKey==="")newFieldKey=fieldKeyParts[i];else newFieldKey=newFieldKey+
...
"-"+rowNum+"_container";removeFromHideFields(container,formId);if(this.classList.contains("wp-editor-area"))removeRichText(this.id)});showAddButton(sectionID);if(typeof frmThemeOverride_frmRemoveRow==="function")frmThemeOverride_frmRemoveRow(id,thisRow);jQuery(document).trigger("frmAfterRemoveRow")});return false}function hideAddButton(sectionID){jQuery("#frm_field_"+sectionID+"_container .frm_add_form_row").addClass("frm_hide_add_button")}function showAddButton(sectionID){jQuery("#frm_field_"+sectionID+
"_container .frm_add_form_row").removeClass("frm_hide_add_button")}function addRow(){var thisBtn,id,i,numberOfSections,lastRowIndex,stateField,state;if(currentlyAddingRow===true)return false;currentlyAddingRow=true;thisBtn=jQuery(this);id=thisBtn.data("parent");i=0;numberOfSections=jQuery(".frm_repeat_"+id).length;if(numberOfSections>0){lastRowIndex=jQuery(".frm_repeat_"+id).last().attr("id").replace("frm_section_"+id+"-","");if(lastRowIndex.indexOf("i")>-1)i=1;else i=1+parseInt(lastRowIndex)}stateField=
document.querySelector('input[name="frm_state"]');state=null!==stateField?stateField.value:"";jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"json",data:{action:"frm_add_form_row",field_id:id,i:i,numberOfSections:numberOfSections,nonce:frm_js.nonce,frm_state:state},success:function(r){if(r.html){var html=r.html;var item=jQuery(html).addClass("frm-fade-in");thisBtn.parents(".frm_section_heading").append(item);if(r.is_repeat_limit_reached)hideAddButton(id);var checked=["other"];var fieldID,fieldObject;
var reset="reset";var repeatArgs={repeatingSection:id.toString(),repeatRow:i.toString()};jQuery(html).find("input, select, textarea").each(function(){if(this.name==="")return true;if(this.type=="file")fieldID=this.name.replace("file","").split("-")[0];else fieldID=this.name.replace("item_meta[","").split("]")[2].replace("[","");if(jQuery.inArray(fieldID,checked)==-1){if(this.id===false||this.id==="")return;fieldObject=jQuery("#"+this.id);checked.push(fieldID);hideOrShowFieldById(fieldID,repeatArgs);
updateWatchingFieldById(fieldID,repeatArgs,"value changed");checkFieldsWithConditionalLogicDependentOnThis(fieldID,fieldObject);checkFieldsWatchingLookup(fieldID,fieldObject,"value changed");doCalculation(fieldID,fieldObject);reset="persist"}});jQuery(html).find(".frm_html_container").each(function(){var fieldID=this.id.replace("frm_field_","").split("-")[0];checked.push(fieldID);hideOrShowFieldById(fieldID,repeatArgs)});loadDropzones(repeatArgs.repeatRow);loadSliders();loadChosen();jQuery(html).find(".frm_html_container").each(function(){var fieldID=
this.id.replace("frm_field_","").split("-")[0];checked.push(fieldID);hideOrShowFieldById(fieldID,repeatArgs)});jQuery(html).find(".wp-editor-area").each(function(){initRichText(this.id)})}if(typeof frmThemeOverride_frmAddRow=="function")frmThemeOverride_frmAddRow(id,r);jQuery(document).trigger("frmAfterAddRow");jQuery(".frm_repeat_"+id).each(function(i){this.style.zIndex=999-i});currentlyAddingRow=false},error:function(){currentlyAddingRow=false}});return false}function triggerToggleClickOnSpace(e){if(32===
...
jQuery(document).on("click",'.frm-show-form input[type="submit"], .frm-show-form input[name="frm_prev_page"], .frm_page_back, .frm_page_skip, .frm-show-form .frm_save_draft, .frm_prev_page, .frm_button_submit',setNextPage);jQuery(document).on("change",'.frm_other_container input[type="checkbox"], .frm_other_container input[type="radio"], .frm_other_container select',showOtherText);jQuery(document).on("click",".frm_remove_form_row",removeRow);jQuery(document).on("click",".frm_add_form_row",addRow);
jQuery(".frm_edit_link_container").on("click","a.frm_inplace_edit",editEntry);jQuery(".frm_edit_link_container").on("click","a.frm_cancel_edit",cancelEdit);jQuery(document).on("click",".frm_ajax_delete",deleteEntry);jQuery(".frm_month_heading, .frm_year_heading").on("click",function(){var content=jQuery(this).children(".ui-icon-triangle-1-e, .ui-icon-triangle-1-s");if(content.hasClass("ui-icon-triangle-1-e")){content.addClass("ui-icon-triangle-1-s").removeClass("ui-icon-triangle-1-e");jQuery(this).next(".frm_toggle_container").fadeIn("slow")}else{content.addClass("ui-icon-triangle-1-e").removeClass("ui-icon-triangle-1-s");
