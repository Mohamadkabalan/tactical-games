...
function SignaturePad (selector, options) {
   ...
   */
  function reDraw() {
    var outputField = $(settings.output, context);
    var outputValue = outputField.val();
    ...
    for ( var i=0, l=outputValue.length; i<l; i++ ) {
      canvasContext.beginPath();
      canvasContext.moveTo(outputValue[i]['mx'], outputValue[i]['my']);
      canvasContext.lineTo(outputValue[i]['lx'], outputValue[i]['ly']);
      canvasContext.lineCap = settings.penCap;
      canvasContext.stroke();
      canvasContext.closePath();
    }
  ...
  function init () {
    ...
    if (element.getContext) {
      ...
      if (!settings.displayOnly) {
        ...
        $(settings.sigNav, context).css('display', 'block');
      }
    }
  }
...
function frmSignatureJS(){
    ...
    function sigInit(opts){
		var frmsigopts = {
			...
			penColour:opts.text_color,
			penWidth: 1.5,
			lineColour: opts.line_color,
			lineWidth: opts.line_width, // Thickness of the signature line
			lineMargin: opts.line_margin, // Margin on right and left of signature line
			lineTop: opts.line_top, // Distance to draw the line from the top
			defaultAction:opts.default_tab,
			validateFields:false,
		};
        var w = opts.width;
        var sigField = jQuery('.frm_field_'+opts.id+'_container:not(.frm_signature), #frm_field_'+opts.id+'_container');
        ...
        if( 0 < w < parseInt(opts.width)){
            ...
            sigPad.find('canvas').attr('width',w-4);
            sigField.find('input[name$="[width]"]').val(w-4);
        }

		sigField.addClass('frm_signature');
	...
	return{
		init: function(){
            ...
            loadSig();
			jQuery( document ).on( 'frmPageChanged', loadSig );
			jQuery( document ).on( 'frmAfterAddRow', loadSig );
			jQuery( document ).on( 'frmFormComplete', loadSig );
        }
    };
}
