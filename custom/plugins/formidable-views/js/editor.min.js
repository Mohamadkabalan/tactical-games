...
layoutData)return layoutData}}return false}function getAllBoxIdsForBoxes(boxes,rowId){var boxIds,boxLength,boxIndex,box;boxIds=[];boxLength=boxes.length;for(boxIndex=0;boxIndex<boxLength;++boxIndex){box=boxes[boxIndex];boxIds=boxIds.concat(getAllBoxIdsForBox(box,rowId))}return boxIds}function getAllBoxIdsForBox(boxData,rowId){var boxIds=[boxData.id];if("undefined"!==typeof boxData.rows)boxIds=boxIds.concat(getAllBoxIdsForRow(rowId,boxData.rows));return boxIds}function createDeleteRowFromLayoutBuilderModalDataFunction(rowId){return function(){deleteRowFromLayoutBuilderModalData(rowId,
...
function getPlusIcon(){var plusIcon=div({icon:"frm_views_editor_plus_circle"});plusIcon.classList.add("frm-add-content-to-layout-block");return plusIcon}function getLayoutClassName(layout){switch(layout){case LAYOUTS.ONE_COLUMN:return"1";case LAYOUTS.TWO_COLUMN:return"1-1";case LAYOUTS.THREE_COLUMN:return"1-1-1";case LAYOUTS.FOUR_COLUMN:return"1-1-1-1";case LAYOUTS.ONE_THREE:return"1-3";case LAYOUTS.THREE_ONE:return"3-1";case LAYOUTS.ONE_TWO_ONE:return"1-2-1"}return layout}function getBoxContentModalContent(boxData){var getWysiwygArea,
getModalBoxSelectArea,getWysiwygSidebar;getWysiwygArea=function(){setBoxDataListeners(boxData);return div({id:"frm_box_content_wysiwyg_wrapper",class:"frm_three_fourths",child:getWysiwyg()})};getModalBoxSelectArea=function(){var layoutFunction;if(!isGridType())return emptyNode();if("undefined"!==typeof boxData.useContent)return emptyNode();if(thereIsOnlyOneBoxOptionToSelect())return emptyNode();layoutFunction=getLayoutFunction({isEditBoxContentModal:true,selectedBoxId:boxData.id,useContent:boxData.useContent});
return layoutFunction({id:0,rows:getActiveLayoutData()})};getWysiwygSidebar=function(){var customizationSidebar=document.getElementById("frm_adv_info");if(state.limitCustomizationSidebar)customizationSidebar.classList.add("frm-limited-customization-sidebar");else customizationSidebar.classList.remove("frm-limited-customization-sidebar");return div({id:"frm_modal_box_select_area",class:"frm_fourth",children:[getModalBoxSelectArea(),customizationSidebar]})};return div({class:["frm_grid_container","frm-education-wrapper"],
children:[getWysiwygArea(),getWysiwygSidebar()]})}function setBoxDataListeners(boxData){jQuery(document).off("change focus","#frm_box_content_wysiwyg_wrapper textarea");jQuery(document).on("change focus","#frm_box_content_wysiwyg_wrapper textarea",getUpdateBoxDataFunction(boxData))}function getWysiwyg(){if(dataProcessor.data.disablePreview)return document.getElementById("postdiv")||document.getElementById("postdivrich");return document.getElementById("postdivrich")||document.getElementById("postdiv")}
function thereIsOnlyOneBoxOptionToSelect(){var rows=getActiveLayoutData();if(rows.length<1)return true;if(rows.length>1)return false;return!rowContainsMoreThanOneBox(rows[0])}function rowContainsMoreThanOneBox(row){var box;if(row.boxes.length>1)return true;if(row.boxes.length<1)return false;box=row.boxes[0];return boxContainsMoreThanOneBox(box)}function boxContainsMoreThanOneBox(box){var length,index,row;if("undefined"===typeof box.rows)return false;length=box.rows.length;for(index=0;index<length;++index){row=
box.rows[index];if(rowContainsMoreThanOneBox(row))return true}return false}function getRawBoxContent(boxData,useBoxContent){var content,boxId,index;content=useBoxContent?state.editBoxContentModalContent:getPageContent(boxData);if("undefined"!==typeof boxData.useContent)return content;boxId=boxData.id;if(0===boxId&&"string"===typeof content)return content;for(index=0;index<content.length;++index)if(boxId===content[index].box)return content[index].content;return""}function getPageContent(boxData){if("undefined"!==
typeof boxData.useContent){if("before_content"===boxData.useContent)return dataProcessor.data.listingBeforeContent;if("after_content"===boxData.useContent)return dataProcessor.data.listingAfterContent}return getActivePageContent()}function getBoxContentModalFooter(boxData){var modalId="frm_edit_box_content_modal";return div({children:[getCancelButton(modalId,true),getUpdateViewButton(boxData,modalId)]})}function getCancelButton(modalId,addRightMargin){var output=button({text:__("Cancel","formidable-views"),
class:"frm-views-modal-cancel-button",id:"frm_cancel_"+modalId,onclickPreventDefault:function(){closeModal(modalId)}});if(addRightMargin)output.style.marginRight="10px";return output}function closeModal(modalId){jQuery("#"+modalId).dialog("close")}function getUpdateViewButton(boxData,modalId){return button({id:"frm_update_box_content_button",onclickPreventDefault:function(){updateBoxContent(boxData);closeModal(modalId)},text:__("Update view","formidable-views")})}function updateBoxContent(boxData){var content=
state.editBoxContentModalContent;if("undefined"!==typeof boxData.useContent){if("before_content"===boxData.useContent)updateBeforeContent(content);else if("after_content"===boxData.useContent)updateAfterContent(content);refreshLayoutBuilderArea()}else updateActivePageContent(content,true);saveViewEditor();refreshActiveViewTypeDescription();addSomeAdditionalViewSettingsEducation()}function updateBeforeContent(content){var disablePreview;dataProcessor.data.listingBeforeContent=content;redrawBeforeAfterContent("before");
disablePreview=document.getElementById("frm_views_disable_preview");if(null!==disablePreview)disablePreview.disabled=isLegacyTableType();updateBeforeContentInput(content)}function updateBeforeContentInput(content){var input;if(isTableType())return;input=document.getElementById("before_content");input.value=content;jQuery(input).trigger("change")}function updateAfterContent(content){dataProcessor.data.listingAfterContent=content;redrawBeforeAfterContent("after")}function redrawBeforeAfterContent(type){var wrapperId,
wrapper;wrapperId="frm_"+type+"_view_content_wrapper";wrapper=document.getElementById(wrapperId);if(null===wrapper)return;redraw(wrapper.parentNode,getBeforeOrAfterContentWrapper(type))}function updateActivePageContent(content,refresh){dataProcessor.data[state.activeTab+"Content"]=content;if(refresh)refreshLayoutBuilderArea()}function refreshLayoutBuilderArea(){boxProcessor.pollServer();redrawLayoutBuilderArea()}function redrawLayoutBuilderArea(){var layoutBuilderArea=document.getElementById("frm_layout_builder_area");
if(null===layoutBuilderArea)return;layoutBuilderArea.innerHTML="";layoutBuilderArea.appendChild(processActivePageContent().firstChild)}function addSomeAdditionalViewSettingsEducation(){maybeAppendEducation(function(){return jQuery(document.getElementById("frm_view_editor_filter_sort_wrapper"))},function($target){appendEducationBelow($target,{title:__("Additional view settings","formidable-views"),description:getAdditionalViewSettingsEducationDescription(),key:"additional-view-settings"})})}function getAdditionalViewSettingsEducationDescription(){if(isGridType())return __("Filter form entries, sort data, and toggle the layout builder at any time.",
"formidable-views");return __("Filter and sort form entries at any time.","formidable-views")}function getLayoutBuilderPlaceholder(){return div({class:["frm-working-space-border","frmcenter"],padding:"30px",child:getLayoutBuilderPlaceholderWrapper()})}function getLayoutBuilderPlaceholderWrapper(){if(onListingTab())return getRepeatableLayoutDescription();return getCustomLayoutDescription(true)}function getCustomLayoutSVG(){var svg=getSVG("custom-layout");svg.style.transform="translateX( -20px )";return svg}
function getCheckmarkListItems(items){return div({children:map(items,getCheckmarkListItem)})}function getCheckmarkListItem(text){return div({class:"frm-checkbox-list-item",children:[getCheckmark(),span({text:text,margin:"0 0 0 7px"})]})}function getCheckmark(){return div({icon:"frm_views_editor_checkmark"})}function deepClone(object){return JSON.parse(JSON.stringify(object))}function redrawLayoutBuilderModalWorkingSpace(){var workingSpace=document.getElementById("frm_layout_builder_modal_working_space");
if(null===workingSpace)workingSpace=document.getElementById("frm_layout_builder_modal_main_area").querySelector(".frm-working-space-border");workingSpace.parentNode.replaceChild(getLayoutBuilderModalWorkingSpace(),workingSpace);makeWorkingAreaDroppable()}function redrawLayoutBuilderFooter(){var modalId,footer;modalId="frm_layout_builder_modal";footer=document.getElementById("frm_layout_builder_modal_footer");jQuery(footer).after(getLayoutBuilderModalFooter(modalId));footer.remove();if(layoutDataIsAlreadySet()&&
""===getActiveLayoutData())jQuery(document).one("mouseover","#frm_layout_builder_modal .frm_modal_footer",function(){addAreYouReadyToAddDataEducation()})}function getLayoutBuilderModalFooter(modalId){var children=[];if(layoutDataIsAlreadySet())children.push(getCancelButton(modalId,true),getSaveLayoutButton(modalId));return div({id:"frm_layout_builder_modal_footer",children:children})}function getSaveLayoutButton(modalId){return button({id:"frm_save_layout",onclickPreventDefault:function(){saveLayout();
closeModal(modalId)},text:__("Save layout","formidable-views")})}function appendToLayoutBuilderModalWorkingSpace(element){document.getElementById("frm_layout_builder_modal_working_space").firstChild.appendChild(element)}function saveLayout(){setActiveLayoutData(state.layoutBuilderModalData);saveViewEditor()}function autoId(){return++currentAutoId}function getBoxAutoId(){if(-1===boxAutoId)if(layoutBuilderModalIsActive())boxAutoId=searchRowsForHighestBoxId(state.layoutBuilderModalData);else boxAutoId=
searchRowsForHighestBoxId(getActiveLayoutData());boxAutoId++;return boxAutoId}function layoutBuilderModalIsActive(){var modal=document.getElementById("frm_layout_builder_modal");return null!==modal&&"none"!==modal.style.display}function searchRowsForHighestBoxId(rows){var index,row,highestBoxIdFromBoxData,length=rows.length,highestBoxId=-1;for(index=0;index<length;++index){row=rows[index];if(row.id>highestBoxId)highestBoxId=row.id;if("undefined"===typeof row.boxes)continue;highestBoxIdFromBoxData=
